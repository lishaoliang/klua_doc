...
```c
// C实现文件: ./klb/src_packages/klbwui/embed_widgets/klbui_line.c
// 窗口实现: ./klb/src_packages/klbwui/embed_wnd/klbwnd_line.c
...
* kline 仅支持 **normal** 状态的样式属性; 无 `:focus` / `:disabled` / `:checked`
* 默认窗口样式: `style-nofocus` + `style-nocommand` + `style-focus-without-redraw`
...
* 无控件私有自定义键; 仅 ① 通用 + ② 盒模型 CSS

### 6. CSS总表

* 先公共后私有; ① 见 `klbui_css_std_function.c`; ②③ 见 `klbui_line.c` `init_func_map`
* 仅 **normal**; 无伪类后缀
...
| 控件通用  | `style-nofocus`                   | get/set | 无聚焦; kline 默认 `true`        |
| 控件通用  | `style-nocommand`                 | get/set | 无 `on_command`; kline 默认 `true` |
| 控件通用  | `style-focus-without-redraw`      | get/set | 聚焦不重绘; kline 默认 `true`    |
...
local color = jq('line1').css('background-color')
```
