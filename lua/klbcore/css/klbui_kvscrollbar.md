...
```c
// C实现文件: ./klb/src_packages/klbwui/embed_widgets/klbui_vscrollbar.c
// 窗口实现: ./klb/src_packages/klbwui/embed_wnd/klbwnd_vscrollbar.c
...
* 先公共后私有; ① 见 `klbui_css_std_function.c`; ②③ 见 `klbui_vscrollbar.c` `init_func_map`
...
| 控件通用  | `style-focus-without-redraw`      | get/set | 聚焦不重绘; kvscrollbar 默认 `true` |
...
| 自定义   | `value`                           | get/set | 滚动位置                        |
| 自定义   | `ranges`                          | get/set | `{min,max,step}`            |
