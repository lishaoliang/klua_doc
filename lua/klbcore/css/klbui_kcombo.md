...
```c
// C实现文件: ./klb/src_packages/klbwui/embed_widgets/klbui_combo.c
// 窗口实现: ./klb/src_packages/klbwui/embed_wnd/klbwnd_combo.c
...
* 下拉项通过 `append` / `clear` 维护; 绑 `/klbui/combo-menu`
...
* 方法: append
* 无状态
* 仅设置; 参数为数组, 每项含 `title` / `value` / `index`

* 设置
```lua
{
    ['append'] = {
        { ['title'] = '选项1', ['value'] = 'opt1', ['index'] = 0 },
        { ['title'] = '选项2', ['value'] = 'opt2', ['index'] = 1 },
    },
}
```
...
* 先公共后私有; ① 见 `klbui_css_std_function.c`; ②③ 见 `klbui_combo.c` `init_func_map`
...
| 命令    | `append`                          | set     | 追加下拉项数组                     |
| 命令    | `clear`                           | set     | 清空下拉项                       |
| 自定义   | `title`                           | get/set | 当前显示文本                      |
| 自定义   | `value`                           | get/set | 当前业务值                       |
| 自定义   | `index`                           | get/set | 当前索引                        |
...
local title = jq('combo1').css('title')
local value = jq('combo1').css('value')
```
