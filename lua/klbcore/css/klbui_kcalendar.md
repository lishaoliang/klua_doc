...
```c
// C实现文件: ./klb/src_packages/klbwui/embed_widgets/klbui_calendar.c
// 窗口实现: ./klb/src_packages/klbwui/embed_shwnd/klbwnd_calendar.c
...
```lua
{
    ['background-image'] = 'cal.bmp',
    ['background-image:focus'] = 'cal_f.bmp',
    ['background-image:disabled'] = 'cal_d.bmp',
}
```
...
* 先公共后私有; ① 见 `klbui_css_std_function.c`; ②③ 见 `klbui_calendar.c` `init_func_map`
...
| 控件通用  | `style-nofocus`                   | get/set | 无聚焦; kcalendar 默认 `true`    |
