...
```c
// C实现文件: ./klb/src_packages/klbwui/embed_widgets/klbui_slider.c
// 窗口实现: ./klb/src_packages/klbwui/embed_wnd/klbwnd_slider.c
...
* 轨道走 `background-color` / `foreground-color` / `foreground-height`; 滑块走 `pos.*`
* 数值走 `value` / `min` / `max`; `title` 为显示文本
...
{
    ['pos.image'] = 'thumb.bmp',
    ['pos.image:focus'] = 'thumb_f.bmp',
}
```

##### 4.8.3 滑块尺寸

* 属性: pos.width / pos.height
* 无状态
...
* 先公共后私有; ① 见 `klbui_css_std_function.c`; ②③ 见 `klbui_slider.c` `init_func_map`
...
| CSS私有 | `foreground-height`               | get/set | 前景条高度                       |
| CSS私有 | `pos.color` + 伪类                  | get/set | 滑块颜色                        |
| CSS私有 | `pos.image` + 伪类                  | get/set | 滑块图片                        |
| CSS私有 | `pos.width` / `pos.height`        | get/set | 滑块尺寸                        |
