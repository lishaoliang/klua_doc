...
```c
// C实现文件: ./klb/src_packages/klbwui/embed_widgets/klbui_time.c
// 窗口实现: ./klb/src_packages/klbwui/embed_wnd/klbwnd_time.c
...
* 绑 `/klbui/decimal-menu`; `value` 为 `{hour,minute,second}` map
...
* 先公共后私有; ① 见 `klbui_css_std_function.c`; ②③ 见 `klbui_time.c` `init_func_map`
...
| 自定义   | `value`                           | get/set | `{hour,minute,second}`      |
