...
```c
// C实现文件: ./klb/src_packages/klbwui/embed_widgets/klbui_edit.c
// 窗口实现: ./klb/src_packages/klbwui/embed_wnd/klbwnd_edit.c
...
* 绑 `/klbui/edit-menu`; `title` 与 `value` 同 handler (别名)
...
* 先公共后私有; ① 见 `klbui_css_std_function.c`; ②③ 见 `klbui_edit.c` `init_func_map`
* 视觉键支持三态: 无后缀 / `:focus` / `:disabled`
* `tip` 由 kedit 覆盖通用实现; 设置 `tip` 会关闭 `dynamic-tip`
...
| 控件通用  | `style-focus-delay`               | get/set | 聚焦延时消息; kedit 默认 `true`   |
...
| 自定义   | `value` / `title`                 | get/set | 编辑文本; 两键同 handler           |
