...
```c
// C实现文件: ./klb/src_packages/klbwui/embed_widgets/klbui_view.c
// 窗口实现: ./klb/src_packages/klbwui/embed_wnd/klbwnd_view.c
...
- kview 仅支持 **normal** 状态的样式属性; 无 `:focus` / `:disabled` / `:checked`
- 默认窗口样式为不可聚焦 (`style-nofocus`); 需要聚焦时设置 `['style-nofocus'] = false`
...
- 先公共后私有; ① 见 `klbui_css_std_function.c`; ②③ 见 `klbui_view.c` `init_func_map`
- 仅 **normal**; 无伪类后缀
...
| 控件通用  | `style-nofocus`                   | get/set | 无聚焦; kview 默认 `true`        |
...
local mode = jq('view1').css('background-image-mode')
local title = jq('view1').css('title')
```
