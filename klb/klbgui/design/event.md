...
> `klua_doc/klb/klbgui/design/event.md` — 头文件: `klb/inc/klbgui/klb_msg.h`, `klb/inc/klbgui/klbui_event.h`; 消息队列: `klb/src_c/klbgui/klb_gui_in.h`
...
| 层 | 头文件 | 内容 |
|----|--------|------|
| 底层输入 | `klb_msg.h` | Windows 风格 `KLB_WM_*`、`KLB_VK_*`；投递到 GUI 消息队列 |
| 框架事件 | `klbui_event.h` | H5 风格命名 `KLBUI_*`；窗口生命周期与控件回调 |
...
`klb_msg_t` 字段：`msg`, `pt1`, `pt2`, `lparam`, `wparam`（定义于 `klb_gui_in.h`）。

**等待状态**：`klb_gui_wait` 启用后 GUI **丢弃**键鼠等外设消息（见 [layer.md](layer.md) § waitlayer）。

## 窗口生命周期

来源 `klbui_event.h` @note：
...
| C | 脚本 |
|----|------|
| `klbui_event.h` / `KLBUI_*` | `klbcore/klbui/event.lua` 字符串名 |
