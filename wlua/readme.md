...
> `klua_doc/wlua/` — 代码: `wlua/` — **klbgui P0a 桌面画布后端**（SDL + wsdl）
...
| 产物 | `klb/bin/klua.exe` | `klb/bin/wlua.exe` |
...
| 模块 | 路径 | 职责 |
|------|------|------|
| `wsdl_wnd` | `wlua/wsdl/` | SDL 窗口与呈现 |
| `wsdl_surface_canvas` | `wlua/wsdl/wsdl_surface_canvas.cpp` | `klb_canvas_t` 软/硬缓冲 |
| `wsdl_surface_canvas_opt` | 同上 | `draw_opt` / FreeType 等 |
| `ft_cache` | `wlua/wsdl/ft_cache.cpp` | 字体光栅 |
...
- VS2015 工程：`wlua/proj/msvc2015/`（ai 禁止读写作源码目录，见 **core-workflow**）
- 产物默认：`klb/bin/wlua.exe`
