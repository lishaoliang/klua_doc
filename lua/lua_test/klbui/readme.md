...
**组织原则**: 按 **手测流程** 分节 (custom UI → 壳/层叠 → 简易控件 → 复合控件), **非** 照搬 API 子模块目录.

**源码目录** (`klua_run/lua_test/`klbui/`): 节子目录 `custom` `shell` `basic` `composite`; 用例 `ch1_s{M}_{z}.lua`（`1.M.z`）; 章共享 `common.lua` (CLI) + `ui.lua` (UI 开窗) + `pref.lua` (`kenv.pref_path("klua","lua_test")` / `klbui.json`) + `theme.lua` (外观 `dark`/`win11-dark`/`ios-dark`/`material-dark`/`github-dark`/`light`/`fluent`); 1.1 模板 `custom/page_tmpl.lua`. 对齐第 2 章形态. **1.1 custom** 与 **1.2.1–1.2.8** / **1.3.3** 已登记; 其余 1.3–1.4 条文 **待实现** (有桩), 不登记.
...
| `a` / `all` | 含本章已登记条 (现行 1.1.1 .. `1.2.1`–`1.2.8`, `1.3.3`) |
...
| 1.2 | [klbui_shell.md](klbui_shell.md) | 壳 / 层叠 | `kdialog` `kview` `ktab` `kmenu` `kdiv`; `modal` / `popup` / `messagebox` |
| 1.3 | [klbui_basic.md](klbui_basic.md) | 简易控件 | `kstatic` `kbutton` `kedit` `kcombo` … |
| 1.4 | [klbui_composite.md](klbui_composite.md) | 复合控件 | `klist` `klistex` `kdate` … |

节号 1.2–1.4 已占; 只追加条, 不提前占 1.5+.
...
| 顶层 type | 页面壳根用 **kview**; `kdialog` 在 1.2 测, 不作壳根 |
...
| 多分辨率适配 | **待定** — 见 **klua-test-design** § 多分辨率 UI 适配 |
| 资源 | `demores/font` `images` `language` (可用 `opts.load_*=false` 关) |
| type | 壳根为 `kview`; `kdialog` 在 1.2 测, 不作 1.1 壳根 |
...
| 1.2 | [klbui_shell.md](klbui_shell.md) | 8 | 8 |
| 1.3 | [klbui_basic.md](klbui_basic.md) | 22 | 1 (`1.3.3`) |
| 1.4 | [klbui_composite.md](klbui_composite.md) | 7 | — (lua 桩 + 文档桩) |
...
| `1.2.1` | `klbui.kdialog` | [klbui_shell.md](klbui_shell.md) |
| `1.2.2` | `klbui.kview` | [klbui_shell.md](klbui_shell.md) |
| `1.2.3` | `klbui.ktab` | [klbui_shell.md](klbui_shell.md) |
| `1.2.4` | `klbui.kmenu` | [klbui_shell.md](klbui_shell.md) |
| `1.2.5` | `klbui.kdiv` | [klbui_shell.md](klbui_shell.md) |
| `1.2.6` | `klbui.modal` | [klbui_shell.md](klbui_shell.md) |
| `1.2.7` | `klbui.popup` | [klbui_shell.md](klbui_shell.md) |
| `1.2.8` | `klbui.messagebox` | [klbui_shell.md](klbui_shell.md) |
| `1.3.3` | `klbui.kpicture` | [klbui_basic.md](klbui_basic.md) |

## 新增用例

1. 在对应节 md 增 `### 1.x.y` (只追加; 1.2–1.4 已占节).
