...
```c
// C实现文件: ./klb/src_packages/klbwui/embed_widgets/klbui_date.c
// 窗口实现: ./klb/src_packages/klbwui/embed_wnd/klbwnd_date.c
...
* 绑 `/klbui/calendar-menu`; `title` 为显示文本; `value` 为 `{year,month,day}` map
...
* 属性: value
* 无状态
* 读写 map: `year` / `month` / `day`

* 设置
```lua
{
    ['value'] = { ['year'] = 2026, ['month'] = 9, ['day'] = 5 },
}
```
...
* 先公共后私有; ① 见 `klbui_css_std_function.c`; ②③ 见 `klbui_date.c` `init_func_map`
...
| 自定义   | `value`                           | get/set | `{year,month,day}`          |
