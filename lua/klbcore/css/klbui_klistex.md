...
```c
// C实现文件: ./klb/src_packages/klbwui/embed_widgets/klbui_listex.c
// 窗口实现: ./klb/src_packages/klbwui/embed_wnd/klbwnd_listex.c
...
* 数据维护走 `append_column` / `append` / `clear` / `clear_data` / `event_wnd`; 选中走 `value`
...
* 先公共后私有; ① 见 `klbui_css_std_function.c`; ②③ 见 `klbui_listex.c` `init_func_map`
* 主体与 `row.*` / `vscrollbar.*` 支持三态; `title.*` / `row-check.*` / `row1|2.*` 无伪类或见 bind

| 分组    | 键                                 | 读写      | 说明                          |
| ----- | --------------------------------- | ------- | --------------------------- |
| 控件通用  | `style-peek-event`                | get/set | 冒泡读消息; klistex 默认 `true`  |
| 控件通用  | `style-focus-without-redraw`      | get/set | 聚焦不重绘; klistex 默认 `true`  |
...
| 命令    | `append_column`                   | set     | 题头列                        |
| 命令    | `append` / `clear` / `clear_data` | set     | 数据行                        |
| 命令    | `event_wnd`                       | set     | 行事件窗口                     |
