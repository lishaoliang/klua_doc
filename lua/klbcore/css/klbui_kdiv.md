...
```c
// C实现文件: ./klb/src_packages/klbwui/embed_widgets/klbui_div.c
// 窗口实现: ./klb/src_packages/klbwui/embed_wnd/klbwnd_div.c
...
* kdiv 仅支持 **normal** 状态的样式属性; 无 `:focus` / `:disabled` / `:checked`
* 默认窗口样式: `style-nofocus` + `style-nocommand`
...
* 先公共后私有; ① 见 `klbui_css_std_function.c`; ②③ 见 `klbui_div.c` `init_func_map`
* 仅 **normal**; 无伪类后缀
...
| 控件通用  | `style-nofocus`                   | get/set | 无聚焦; kdiv 默认 `true`         |
| 控件通用  | `style-nocommand`                 | get/set | 无 `on_command`; kdiv 默认 `true` |
