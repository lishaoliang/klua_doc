...
```c
// C实现文件: ./klb/src_packages/klbwui/embed_widgets/klbui_qrcode.c
// 窗口实现: ./klb/src_packages/klbwui/embed_wnd/klbwnd_qrcode.c
...
* kqrcode 仅支持 **normal** 状态的样式属性; 无 `:focus` / `:disabled` / `:checked`
* 默认窗口样式: `style-nofocus` + `style-focus-without-redraw`
* 无文本色 / 字号 / 背景图绑定; 仅 `margin` / `padding` (合并键) / `background-color` / `border-*`
* `value` 为编码内容; set 时触发重绘二维码
...
* 先公共后私有; ① 见 `klbui_css_std_function.c`; ②③ 见 `klbui_qrcode.c` `init_func_map`
* 仅 **normal**; 无伪类后缀
...
| 控件通用  | `style-nofocus`                   | get/set | 无聚焦; kqrcode 默认 `true`     |
| 控件通用  | `style-focus-without-redraw`      | get/set | 聚焦不重绘; kqrcode 默认 `true` |
| CSS公共 | `margin`                          | get/set | 仅合并键                       |
| CSS公共 | `padding`                         | get/set | 仅合并键                       |
| CSS公共 | `background-color`                | get/set | 背景填充                       |
| CSS公共 | `border-width`                    | get/set | 边框宽度                       |
| CSS公共 | `border-color`                    | get/set | 边框颜色                       |
| 自定义   | `title`                           | get/set | 标识字符串                      |
| 自定义   | `value`                           | get/set | 编码内容; set 重绘               |
