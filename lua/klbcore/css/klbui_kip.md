...
```c
// C实现文件: ./klb/src_packages/klbwui/embed_widgets/klbui_ip.c
// 窗口实现: ./klb/src_packages/klbwui/embed_wnd/klbwnd_ip.c
...
* 绑 `/klbui/decimal-menu-ip`; `title` 与 `value` 同 handler (别名)
...
* 先公共后私有; ① 见 `klbui_css_std_function.c`; ②③ 见 `klbui_ip.c` `init_func_map`
...
| 自定义   | `value` / `title`                 | get/set | IP 字符串; 两键同 handler         |
