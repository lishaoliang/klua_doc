...
```c
// C实现文件: ./klb/src_packages/klbwui/embed_widgets/klbui_btnex.c
// 窗口实现: ./klb/src_packages/klbwui/embed_wnd/klbwnd_btnex.c
...
```lua
{
    ['background-image'] = 'btnex.bmp',
    ['background-image:focus'] = 'btnex_f.bmp',
    ['background-image:disabled'] = 'btnex_d.bmp',
}
```
...
* 先公共后私有; ① 见 `klbui_css_std_function.c`; ②③ 见 `klbui_btnex.c` `init_func_map`
...
| 命令    | `graph`                           | get/set | 绘制类型字符串                     |
| 自定义   | `title`                           | get/set | 显示文本                        |
| 自定义   | `value` / `value-str`             | get/set | 字符串业务值                      |
| 自定义   | `index`                           | get/set | 索引                          |
| 自定义   | `value-int`                       | get/set | 整型值                         |
| 自定义   | `value-bool`                      | get/set | 布尔值                         |
...
local graph = jq('btnex1').css('graph')
```
